...
const posts = [
  new Post({ name: 'Varun', title: 'Hello Udacity', slug: 'hello-udacity', cuid: 'f34gb2bh24b24b2', content: "All cats meow 'Udacity!'" }),
  new Post({ name: 'Rahul', title: 'Hi Google', slug: 'hi-google', cuid: 'f34gb2bh24b24b3', content: "All dogs bark 'Google!'" }),
];
...
test.serial('Should send correct data when queried against a cuid', async t => {
  ...
  const post = new Post({ name: 'Foo', title: 'bar', slug: 'bar', cuid: 'f34gb2bh24b24b2', content: 'Hello Udacity! says Foo' });
  post.save();
...
test.serial('Should correctly add a post', async t => {
  ...
  const res = await request(app)
    .post('/api/posts')
    .send({ post: { name: 'Foo', title: 'bar', content: 'Hello Udacity says Foo' } })
    .set('Accept', 'application/json');
